...
정책값은 TrackcropConfig에서 읽는다(cfg 미지정 시 기본값).
debug 리스트를 주면 시점별 선택 공/소유선수 bbox를 수집한다(대상 하이라이트용).
"""

from .config import TrackcropConfig
from .constants import CENTER_FALLBACK_X, CROP_WIDTH, SOURCE_WIDTH
...
def _bbox(det: Detection | None) -> list[float] | None:
    """Detection → [x, y, w, h] (디버그 직렬화용). None이면 None."""
    if det is None:
        return None
...
def _select_carrier(
    ...
    def score(player: Detection) -> float:
        ...
        if pv is not None and abs(ball_velocity) > 1e-9:
            velocity_sim = 1.0 - min(abs(pv - ball_velocity) / cfg.velocity_scale, 1.0)
        return cfg.w_carrier_proximity * proximity + cfg.w_carrier_velocity * velocity_sim
...
def resolve_targets(
    ...
    for offset_ms, detections in samples:
        ...
        ball = select_ball(
            balls,
            tracker.last_seen_x,  # 게이트·연속성 기준 = 마지막 실측(예측 드리프트 배제)
            player_centers,
            prev_ball_id,
            cfg,
            tracker.gate_radius(offset_ms),
        )
        ...
        if ball is not None and players:
            cid = _select_carrier(players, ball, tracker.velocity, player_velocity, cfg)
            if cid is not None:
                carrier_id = cid
        ...
        if ball_x is not None:
            if ball is not None:
                ...
                confidence = last_confidence * cfg.confidence_decay
            ...
            if carrier is not None:
                ...
                and offset_ms - last_observed_ms <= cfg.player_lost_hold_ms
            ):
        ...
        if debug is not None:
            # 대상 하이라이트용 — 이 시점 선택된 공/소유선수의 실제 bbox
            debug.append(
                {
                    "video_offset_ms": offset_ms,
                    "ball_bbox": _bbox(ball),
                    "carrier_bbox": _bbox(_find_by_track_id(players, carrier_id)),
                }
            )
