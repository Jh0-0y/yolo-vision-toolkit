"""라벨과 클래스 — YOLO 라벨 파일 형식, 데이터셋 라벨 저장소, 클래스 목록.

    io              라벨 .txt · box meta · data.yaml 읽고 쓰기 (좌표 변환 포함)
    store           데이터셋 디렉터리 기준 박스 · 검수 상태
    classes         클래스 목록 CRUD (추가 · 이름변경 · 삭제)
    split           검수완료를 train/val/test 로 나누기
    import_yolo     YOLO zip 가져오기 (클래스 병합 · id 재매김)
    dataset_export  데이터셋 -> YOLO 트리 · zip
    registry        여러 모델의 클래스 이름을 하나로 합치기 (앙상블 · 비교)

`io` 가 바닥이다 — `store` 와 `classes` 가 그 위에 데이터셋 규약(labels/{stem}.txt ·
