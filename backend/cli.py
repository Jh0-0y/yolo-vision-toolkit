...
Usage:
    uv run python cli.py label --models a.pt b.pt --images ./imgs --out ./out \
        [--conf 0.4] [--iou-wbf 0.55] [--device auto]
"""
...
def main() -> None:
    ...
    label.add_argument("--out", required=True, help="output folder (labels/ + classes.json)")
    label.add_argument("--conf", type=float, default=0.4)
    ...
    if args.command == "label":
        ...
        def progress(ev: dict) -> None:
            if ev.get("phase") == "inference":
                print(
                    f"\r[{ev['done']}/{ev['total']}] boxes={ev['boxes']}",
                    end="",
                    flush=True,
                )
