...
Crop Draw 는 `detect_id`·마지막 렌더 잡 id·그때 돌려 둔 노브만 브라우저에 남기고
(`cropDrawSession.ts`), 검출 결과·궤적은 담지 않는다 — 서버 캐시에서 다시 받는다. 그 캐시가
아직 살아 있는지는 `GET /predict/live/{id}/status` 만이 안다(`expired` 면 TTL 로 쓸려 나간 것,
`has_render` 면 구워 둔 오버레이 영상이 아직 있는 것). 원본 File 은 남지 않으므로
**재분석하려면 영상을 다시 올려야 한다.**

검출 패스와 오버레이 렌더는 **둘 다 전역 잡 카드**가 진행률을 보여준다. 이 화면은 시작만 하고
끝났다는 신호로 결과를 가져온다 — 인페이지 진행바를 다시 만들지 않는다.
