...
Crop Draw 는 `detect_id` 와 그때 돌려 둔 노브만 브라우저에 남기고(`cropDrawSession.ts`),
검출 결과·궤적은 담지 않는다 — 서버 캐시에서 다시 받는다. 그 캐시가 아직 살아 있는지는
`GET /predict/live/{id}/status` 만이 안다(`expired` 면 TTL 로 쓸려 나간 것). 원본 File 은
남지 않으므로 **재분석하려면 영상을 다시 올려야 한다.**
