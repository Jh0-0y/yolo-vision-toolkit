...
상태를 **그 자리에서 만든다**(`services/crop_runs.py`). 산출물 디렉터리가 곧 목록이고 진행
...
- 열거할 수 없는 것(라이브 프리뷰 캐시처럼)은 **id 만 브라우저에 적어 두고, 아직 쓸 수 있는지는 서버에 묻는다**(`/predict/live/{id}/status`). 캐시는 TTL 로 사라지므로 클라이언트가 기억한 것만으로는 알 수 없다.
