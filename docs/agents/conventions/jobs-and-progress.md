...
**잡을 새로 만들면 이 복구를 같이 만들어야 한다.** 지금 복구가 있는 건 학습과 크롭 런 둘이다.

## 상태를 아예 저장하지 않는 길

DB 에 적지 않으면 어긋날 것도 없다. 크롭 런이 이 길을 간다 — 목록 API 가 `progress.jsonl` 의
마지막 종료 이벤트를 읽어 `running`/`done`/`error` 를 **그 자리에서 만든다**(`services/crop_runs.py`).

- 산출물 디렉터리가 곧 목록이고, 진행 현황도 같은 목록이다. 이어붙일 두 번째 진실이 없다.
- 워커 풀은 API 프로세스가 소유하므로 재시작하면 돌던 잡은 죽는다 — 부팅 때 종료 이벤트를
...
- 서버가 **도는 잡을 스스로 열거할 수 있으면** localStorage 를 쓰지 않는다. 크롭 런은 `syncCropRuns(projectId)` 가 목록을 물어 다시 붙는다 — 다른 브라우저·다른 기기에서도 같은 현황이 보인다.
