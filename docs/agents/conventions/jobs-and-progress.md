...
크롭 런(`lab_crop_runs.reconcile_on_boot`), 타일링(`tile_manager.reconcile_on_boot`) 셋이다.
...
- **전역 잡 카드에 모든 잡을 올리지 않는다.** 결과를 볼 자리가 따로 있는 잡은 그 화면이 직접
  ...
  카드를 쓰는 것은 **결과를 볼 자리가 없는** 잡뿐이다 — 지금은 영상 가져오기(`import`)와
  오토라벨링(`autolabel`) 둘. 크롭 런과 학습은 각자 상세 페이지가 진행률을 보여주므로 여기 없다.
  타일링(`tiling`)도 카드를 쓴다 — 결과(파생 데이터셋)를 볼 자리는 있지만 **사용자는
  원본 데이터셋 화면에 그대로 머무르므로**, 진행률을 볼 자리가 거기 말고 없다.
  대신 완료 카드가 파생 데이터셋으로 가는 링크를 준다.
